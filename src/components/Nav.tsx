...
export function Nav() {
  ...
  return (
    <header className={clsx('fixed top-0 left-0 right-0 z-50 transition-all duration-300',
        ...
        <nav className="hidden md:flex items-center gap-1">
          {links.map((link) => (
            <Link key={link.href} to={link.href} className={clsx(
              'px-4 py-2 rounded-lg font-body font-500 text-sm transition-all',
              link.href === '/admin'
        ...
        <div className="hidden md:flex items-center gap-2">
          {isAuthenticated ? (
            ...
            <>
              ...
              <Link to="/signup" className="btn-primary text-sm py-2 px-5"><Zap className="w-3.5 h-3.5" />Get Pro</Link>
            </>
          )}
        </div>

        <button className="md:hidden p-2 text-ink-200" onClick={() => setOpen(!open)}>
      ...
      {open && (
        <div className="md:hidden bg-ink-900/95 backdrop-blur-xl border-b border-ink-800">
          <nav className="max-w-7xl mx-auto px-4 py-4 flex flex-col gap-1">
            ...
            {isAuthenticated && (
              <Link to="/profile" className="flex items-center gap-2 px-4 py-3 rounded-xl text-ink-200 hover:bg-ink-800 font-500">
            ...
            <div className="mt-3 pt-3 border-t border-ink-800 flex flex-col gap-2">
              {isAuthenticated
                ...
                : (<><Link to="/login" className="btn-secondary text-sm text-center">Log in</Link>
                    <Link to="/signup" className="btn-primary text-sm justify-center"><Zap className="w-3.5 h-3.5" />Get Pro</Link></>)}
            </div>
          </nav>
        </div>
      )}
